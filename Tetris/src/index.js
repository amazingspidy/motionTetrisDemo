  ...
  setInterval(async () => {
    ...
    if (leftAngleDelta > rightAngleDelta) {
      if (
        ...
        leftWristX > prevLeftWristX
      ) {
        eventElement.textContent = "왼쪽 회전";
        // rotateDegree -= 30;  // 30도 좌측회전
        rotateCompound(activeBlock, -Math.PI / 12); // 숫자 늘릴수록 적게 회전전
      } else {
      ...
      if (
        ...
        rightWristX < prevRightWristX
      ) {
        eventElement.textContent = "오른쪽 회전";
        //rotateDegree += 30;  // 30도 우측회전
        rotateCompound(activeBlock, Math.PI / 12);
      } else {
        eventElement.textContent = "유지";
      }
    }
    ...
    leftAngleElement.textContent = `Left Angle: ${leftAngleInDegrees.toFixed(
      2
    )} degrees`;

    let rightAngleElement = document.getElementById("rightAngle");
    rightAngleElement.textContent = `Right Angle: ${rightAngleInDegrees.toFixed(
      2
    )} degrees`;
    ...
    rightWristXElement.textContent = `Right Wrist X: ${rightWrist.x.toFixed(
      2
    )}`;
  }, 250);
}
